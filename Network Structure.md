...
### ResNet
解决梯度消失的问题，下一层的输入是当前层的输出加上原始输入数据（实验验证加到激活函数前效果更好）
...
### attention
Query、Key、Value三个矩阵
Q表示正在处理的目标词，K表示输入序列中其他词的标识，V表示输入序列中所有其他词的真实语义信息

1. 当前词的Q和所有词的K进行点积表示相关性，分数越高表示当前词越关注对应的词
2. 缩放，防止分数过大导致梯度消失，将分数除以\sqrt{d_k} ,d_k是向量维度
3. 归一化：softmax，转换为注意力概率权重
4. 加权求和：将得到的权重乘以V，最后全部相机，得到的便是当前词的新语义表示，虽包含了所有词，但也会重点关注权重最高的词

self-attention：自注意力，QKV来自输入句子本身，常用于auto-encoder

cross-attention：交叉注意力，Q来自decoder，KV来自encoder

mask-attention：Q来自当前词，KV只允许看到当前词与之前的词，常用于大语言模型生成文本，保证逐字预测

### 多头注意力
相当于有多个channel，每个channel的关注点不同

### 位置编码
注意力机制下每个词语的位置对结果没有影响，相当于放哪都无所谓，需要有对位置信息的额外认识

早期的用的是正弦波，现在主流大模型（如Llama）则使用旋转位置编码（RoPE）。

### BERT
BERT（Bidirectional Encoder Representations from Transformers）是Google AI团队在2018年提出的一种自然语言处理（NLP）模型。它通过在大规模无标签文本上进行预训练，学习文本的深度双向表示，其影响力被誉为NLP领域的“ImageNet时刻”，标志着NLP正式迈入“深度上下文理解”的新纪元。

BERT的核心创新在于实现了真正的深度双向性（Deep Bidirectional）。它在处理一个词时，能同时利用其左侧和右侧的上下文信息，从而更准确地理解词义，尤其是解决“一词多义”问题。

为了实现这种双向性，BERT引入了两个关键的预训练任务：

- 掩码语言模型（Masked Language Model, MLM）：随机遮蔽输入句子中15%的词，让模型根据上下文预测被遮蔽的词是什么。这强迫模型深度理解整个句子的语境。
- 下一句预测（Next Sentence Prediction, NSP）：输入两个句子（A和B），让模型判断B是否是A的下一句。这使模型具备了理解句子间逻辑关系的能力。

BERT的输入是一个特殊的序列，其构造融合了三种嵌入信息的总和：

- Token Embeddings：词本身的向量表示，使用WordPiece分词。
- Segment Embeddings：用于区分两个句子（如问题和答案）。
- Position Embeddings：记录词的位置信息。

此外，每个输入序列开头都有一个特殊的 [CLS] 标记，其对应的最终输出向量常被用作整个序列的聚合表示，用于分类任务。句子间则用 [SEP] 标记分隔。

#### 预训练-微调范式

预训练：在大规模无标签文本（如维基百科）上通过MLM和NSP任务进行自监督学习，学到通用的语言知识。

微调：将预训练好的BERT模型参数作为初始值，在特定下游任务（如情感分析、问答系统）的少量标注数据上进行训练。这种方式能以较低成本获得极佳效果。

### Vision Transformer ViT
把图像切成好几个patch， patch+position embedding
